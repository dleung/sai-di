class SearchesController < ApplicationController
  def index
    ...
    if params[:tag_ids].blank?
      @articles = Article.published.where(conditions, params[:q], params[:q]).order("created_at DESC").page(params[:page]).per(10)
      ...
      @articles = Article.tagged_with(params[:tag_ids]).published.where(conditions, params[:q], params[:q]).order("created_at DESC").page(params[:page]).per(10)
      @search_tags = params[:tag_ids]
    end
